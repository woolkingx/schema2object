...
fn simple_defaults() {
    ...
    let sv = ObjectTree::new(json!({}), schema).with_defaults();
...
fn existing_values_not_overwritten() {
    ...
    let sv = ObjectTree::new(json!({"status": "active"}), schema).with_defaults();
...
fn nested_defaults() {
    ...
    let sv = ObjectTree::new(json!({"config": {}}), schema).with_defaults();
...
fn nested_partial_existing() {
    ...
    let sv = ObjectTree::new(json!({"config": {"retries": 10}}), schema).with_defaults();
...
fn without_schema_no_op() {
    let sv = ObjectTree::without_schema(json!({})).with_defaults();
    assert!(sv.is_empty());
}
...
fn non_object_data_unchanged() {
    ...
    let sv = ObjectTree::new(json!(42), schema).with_defaults();
...
fn mixed_defaults_and_existing() {
    ...
    let sv = ObjectTree::new(json!({"b": "world"}), schema).with_defaults();
