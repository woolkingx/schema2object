...
use schema2object::{ErrorKind, ObjectTree};
use serde_json::json;
...
#[test]
fn ref_resolves_local_definition() {
    let schema = json!({
        "definitions": {
    ...
    assert_valid(schema.clone(), json!(10));
    assert_invalid(schema, json!(-1), ErrorKind::Minimum);
}
...
#[test]
fn min_length_grapheme_cluster() {
    let schema = json!({"type": "string", "minLength": 1, "maxLength": 1});
    // single grapheme cluster (family emoji) should count as length 1
