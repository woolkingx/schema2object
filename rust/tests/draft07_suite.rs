...
fn run_file(path: &Path) -> (usize, usize, Vec<String>) {
    ...
    for group in &groups {
        for case in &group.tests {
            let sv = SchemaValue::new(case.data.clone(), group.schema.clone());
            let result = sv.validate();
            ...
            if actual_valid == case.valid {
                ...
                failures.push(format!(
                    ...
                    if let Err(errs) = &result {
                        format!("(errors: {:?})", errs.iter().map(|e| e.to_string()).collect::<Vec<_>>())
                    } else {
                        String::new()
                    }
                ));
...
fn draft07_test_suite() {
    ...
    assert!(
        rate >= 90.0,
        "Draft-07 pass rate {rate:.1}% is below 90% threshold ({total_fail} failures)"
    );
}
