...
fn one_of_selects_unique_branch() {
    ...
    let sv = ObjectTree::new(json!({"kind": "circle", "radius": 5}), schema);
...
fn one_of_fails_on_zero_matches() {
    ...
    let sv = ObjectTree::new(json!({"x": "z"}), schema);
...
fn one_of_fails_on_multiple_matches() {
    ...
    let sv = ObjectTree::new(json!({"x": 42}), schema);
    assert!(sv.one_of().is_err());
}
...
fn any_of_fails_on_no_matches() {
    ...
    let sv = ObjectTree::new(json!({"x": 99}), schema);
...
fn all_of_merges_schemas() {
    ...
    let sv = ObjectTree::new(json!({"id": 1, "name": "Alice", "age": 30}), schema);
...
fn not_of_rejects_matching() {
    ...
    let sv = ObjectTree::new(json!("hello"), schema);
    assert!(!sv.not_of());
}
...
fn not_of_accepts_non_matching() {
    let schema = json!({"not": {"type": "string"}});
    let sv = ObjectTree::new(json!(42), schema);
    assert!(sv.not_of());
}

#[test]
fn not_of_with_explicit_schema() {
    let sv = ObjectTree::without_schema(json!([1, 2]));
    assert!(sv.not_of());
}
...
fn if_then_follows_then_branch() {
    ...
    let sv = ObjectTree::new(json!({"country": "US", "state": "CA"}), schema);
...
fn if_then_follows_else_branch() {
    ...
    let sv = ObjectTree::new(json!({"country": "CA", "province": "ON"}), schema);
    let result = sv.if_then();
...
fn if_then_no_if_returns_self() {
    ...
    let sv = ObjectTree::new(json!({"x": 1}), schema);
    let result = sv.if_then();
    assert_eq!(result, sv);
}
...
fn project_auto_resolves_one_of() {
    ...
    let sv = ObjectTree::new(json!({"type": "a", "x": 1, "extra": true}), schema);
    let projected = sv.project().unwrap();
    let data = projected.to_value();
    let obj = data.as_object().unwrap();
...
fn contains_finds_matching_element() {
    ...
    let sv = ObjectTree::new(json!(["user", "admin", "guest"]), schema);
...
fn contains_with_explicit_schema() {
    let sv = ObjectTree::without_schema(json!([1, "hello", true]));
...
fn one_of_branch_then_validate() {
    ...
    let sv = ObjectTree::new(data, schema);
    let branch = sv.one_of().unwrap();
...
fn all_of_merge_then_validate() {
    ...
    let sv = ObjectTree::new(json!({"name": "Alice", "age": 30}), schema);
    let merged = sv.all_of();
