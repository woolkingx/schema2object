...
fn construct_from_value() {
    let sv: SchemaValue = json!(42).into();
    assert!(sv.is_number());
    assert!(sv.schema().is_none());
}
...
fn into_inner_round_trip() {
    ...
    let sv = SchemaValue::without_schema(original.clone());
...
fn type_checks_all_variants() {
    ...
    assert!(SchemaValue::without_schema(json!(42)).is_number());
...
#[test]
fn extractors() {
    assert_eq!(SchemaValue::without_schema(json!(true)).as_bool(), Some(true));
    assert_eq!(SchemaValue::without_schema(json!(42)).as_i64(), Some(42));
    assert_eq!(SchemaValue::without_schema(json!(42)).as_u64(), Some(42));
    assert_eq!(SchemaValue::without_schema(json!(3.14)).as_f64(), Some(3.14));
...
fn display_shows_json() {
    let sv = SchemaValue::without_schema(json!({"k": 42}));
    let s = sv.to_string();
...
fn serialize_round_trip() {
    ...
    let sv = SchemaValue::without_schema(data.clone());
    let json_str = serde_json::to_string(&sv).unwrap();
    let recovered: SchemaValue = serde_json::from_str(&json_str).unwrap();
...
fn get_index_oob_returns_none() {
    let sv = SchemaValue::without_schema(json!([1]));
...
fn len_and_empty() {
    ...
    let empty_obj = SchemaValue::without_schema(json!({}));
...
fn entries_iterates_object() {
    let sv = SchemaValue::without_schema(json!({"x": 1, "y": 2}));
...
fn validate_full_object() {
    ...
    let good = SchemaValue::new(json!({"name": "Alice", "age": 30}), schema.clone());
    assert!(good.validate().is_ok());

    let bad = SchemaValue::new(json!({"name": "Alice"}), schema);
