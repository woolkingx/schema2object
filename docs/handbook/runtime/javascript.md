...
Repository posture: this is the active runtime surface. The schema-as-object semantics from `v0.6.1` remain the user-facing model; `v0.7.0` separates the validation boundary from the cursor constructor so external data enters through `ObjectTree.from(...)` or `validate(...)`.

## Chapter Role

| Field | Value |
|---|---|
| Input artifact | JavaScript runtime question after the usage posture is known |
| Output artifact | API surface for `ObjectTree`, `Loader`, `validate()`, and `$` methods |
| Owner | `js/schema2object.mjs` |
| Gate | `node js/tests/validate.mjs` and `node js/tests/draft07_suite.mjs` |
| Failure route | Return to `runtime/schema2object-usage.md` if the question is operational usage; return to code/tests if the API description disagrees with runtime behavior |
...
const user = ObjectTree.from({ name: 'Alice' }, schema)
...
## Boundary Entry And Cursor

```javascript
// External data boundary: validate, then return a cursor.
const tree = ObjectTree.from(data, schema)

// Internal cursor over already-legal data.
const cursor = new ObjectTree(data, schema)
...
const tree = ObjectTree.from(data, schema, './schemas/')
const tree = ObjectTree.from(data, schema, { 'http://example.com/addr.json': addrSchema })
const tree = ObjectTree.from(data, schema, (uri) => fetchSchema(uri))
```
...
const user = ObjectTree.from(data, schema)

// If schema uses relative $ref, pass base dir as resolver
const user = ObjectTree.from(data, schema, './schemas/')
```
...
const user = ObjectTree.from({ address: { city: 'NY', zip: '10001' } }, schema)
...
const t = ObjectTree.from({ name: 'Alice' }, schema)
...
const user = ObjectTree.from(data, schema, (uri) => {
...
const user = ObjectTree.from({ name: 'Alice', age: 30 }, schema)
...
See [observer_schema_context.mjs](../../../js/examples/observer_schema_context.mjs) for complete examples.
...
- [Usage Guide](schema2object-usage.md) — Loader/resolve patterns, dot key support, observer hooks, common mistakes
- [Language quick start](../language/quick-start.md) — optional projection guidance, not active runtime usage
- [TypeScript Ajv projection](../language/typescript-ajv.md) — optional projection guidance
- [Python projection guide](../language/python.md) — optional projection guidance
- [Rust projection guide](../language/rust.md) — optional projection guidance
- [Handbook index](../index.html)
