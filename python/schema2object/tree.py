...
class ObjectTree(SchemaAPI, MutableMapping):
    ...
    def _apply_defaults(data: Mapping, schema: 'ObjectTree') -> dict | Mapping:
        ...
        # Recurse into nested objects for defaults
        for k, v in data.items():
            if isinstance(v, Mapping):
                sub_schema = props.get(k) if isinstance(props, Mapping) else None
                if isinstance(sub_schema, Mapping):
                    data[k] = ObjectTree._apply_defaults(v, ObjectTree(sub_schema))
        return data
        ...
        if isinstance(data, dict):
            ...
            if isinstance(schema, ObjectTree):
                ...
                if isinstance(props, Mapping):
                    return {k: self._unwrap(data[k]) for k in props if k in data}
