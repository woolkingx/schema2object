...
import math
import re
from .grapheme import grapheme_len
...
def _check_value(schema: Mapping, value: Any) -> tuple[bool, str]:
    ...
    if isinstance(value, str):
        ...
        if mnl is not None and length < _raw(mnl):
            return False, f"length {length} < minLength {_raw(mnl)}"

        mxl = schema.get('maxLength')
        if mxl is not None and length > _raw(mxl):
            return False, f"length {length} > maxLength {_raw(mxl)}"
